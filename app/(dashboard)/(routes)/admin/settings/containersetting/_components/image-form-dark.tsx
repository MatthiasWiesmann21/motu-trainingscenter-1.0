...
interface ImageFormDarkProps {
  initialData: Container;
  containerId: string;
}

const formSchema = z.object({
  imageUrlDark: z.string().min(1, {
    message: "Image is required",
  }),
});

export const ImageFormDark = ({
  initialData,
  containerId,
}: ImageFormDarkProps) => {
  ...
  return (
    ...
    <CardHeader>
      <CardTitle className="flex items-center text-xl justify-between">
        <span>{currentLanguage.customize_ImageFormDark_title}</span>
        <Button
          onClick={toggleEdit}
          variant="ghost"
          size="sm"
    ...
    {isEditing && (
      <CardFooter className="flex justify-between">
        ...
        <Button
          onClick={() => onSubmit({ imageUrlDark: initialData.imageUrlDark || "" })}
          disabled={!initialData.imageUrlDark}
        >
