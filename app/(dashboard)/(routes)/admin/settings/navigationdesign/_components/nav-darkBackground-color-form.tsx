  ...
  return (
    ...
    <CardHeader>
      <CardTitle className="flex items-center justify-between space-x-2">
        <div className="flex text-xl space-x-2">
          ...
          <span>{currentLanguage.customize_navDarkBackgroundColorForm_title}</span>
        </div>
        <Button
          onClick={toggleEdit}
          variant="ghost"
    ...
    <CardContent>
      {!isEditing && (
        ...
        <div className="w-10 h-10 rounded-md border border-gray-500" style={{backgroundColor: initialData.navDarkBackgroundColor}}/>
        <p className="text-md font-medium">{initialData.navDarkBackgroundColor}</p>
        </div>
      )}
      {isEditing && (
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{currentLanguage.customize_navDarkBackgroundColorForm_ColorLabel}</FormLabel>
                  <FormControl>
                    <div className="flex items-center space-x-4">
                      <input
                        type="color"
