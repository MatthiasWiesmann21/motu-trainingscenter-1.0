  ...
  return (
    <div>
      <div className="flex items-center py-4 justify-between">
        <Input
          placeholder={currentLanguage.usergroups_search_placeholder_text}
        ...
        {(isAdmin || isClientAdmin) && (
            <Button className="w-64" onClick={() => onOpen("createUsergroup")}>
              <PlusCircle className="h-4 w-4 mr-2" />
              {currentLanguage.usergroups_createUsergroup_button_text}
            </Button>
        )}
      </div>
            ...
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center">
                  {currentLanguage.usergroups_noUsergroups_text}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
