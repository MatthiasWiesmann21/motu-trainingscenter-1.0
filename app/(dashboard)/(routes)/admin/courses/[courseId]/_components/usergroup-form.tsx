  ...
  return (
    <div className="mt-6 border bg-slate-200 dark:bg-slate-700 rounded-md p-4">
      <div className="font-medium flex items-center justify-between">
        {currentLanguage.courses_UsergroupForm_title}
        <Button onClick={toggleEdit} variant="ghost">
          {isEditing ? (
            <>{currentLanguage.courses_UsergroupForm_cancel}</>
          ) : (
            <>
              <Pencil className="h-4 w-4 mr-2" />
              {currentLanguage.courses_UsergroupForm_edit}
            </>
          )}
        </Button>
      </div>
      {!isEditing && (
        <p className={cn(
          ...
          {selectedOption?.label || `${currentLanguage.courses_UsergroupForm_allUsers}`}
        </p>
      )}
      {isEditing && (
        <Form {...form}>
          ...
          >
            ...
            <div className="flex items-center gap-x-2">
              ...
              >
                {currentLanguage.courses_UsergroupForm_save}
              </Button>
            </div>
          </form>
        </Form>
