...
import { Calendar, CalendarPlus, Info, MoreVertical, Pencil, PlayCircle, Share2, Star, Trash } from "lucide-react";
...
export const EventCard = ({
  ...
  const isLive =
    ...
    new Date(new Date(endDateTime)?.setMinutes(new Date(endDateTime)?.getMinutes() + 1)) >= new Date()
        ...
        <div className="group h-full overflow-hidden rounded-lg bg-white p-2 transition hover:shadow-lg dark:border-[#1f182b] dark:bg-[#0c0319]">
          ...
          <Link href={`/live-event/${id}`} className="relateive flex">
            ...
            <div className="flex hidden max-h-44 w-1/3 flex-col items-center justify-center bg-slate-100 p-2 py-6 text-center dark:bg-gray-800 md:block md:rounded-l-none md:rounded-r-md">
              <p className="text-xl font-bold">{moment(startDateTime).format("DD")}</p>
              <p className="text-lg font-semibold">{moment(startDateTime).format("MMM")}</p>
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="mr-2">
              {category && <CategoryItemCard label={category} colorCode={categoryColorCode} />}
            </div>
            <div className="flex items-center justify-between">
              ...
              <Button onClick={handleCalendarClick} variant="ghost" className="h-8 w-8 p-0">
          ...
          <Link className="" href={`/live-event/${id}`}>
            <Tooltip>
              <TooltipTrigger>
                <div className="line-clamp-2 py-1 text-start text-sm font-medium md:text-base">{title}</div>
              </TooltipTrigger>
              <TooltipContent side="bottom">
                <div className="whitespace-normal text-sm font-medium">{title}</div>
              </TooltipContent>
            </Tooltip>
            <div className="mt-2 block flex items-center justify-center gap-2 rounded-md bg-slate-100 py-2 text-center dark:bg-gray-800 md:hidden">
              <p className="text-md font-bold">{moment(startDateTime).format("DD")}</p>
              <p className="text-sm font-medium">{moment(startDateTime).format("MMM")}</p>
